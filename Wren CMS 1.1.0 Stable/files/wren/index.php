...
function menu_html(string $activeSlug = ''): string
{
    ...
    $st = db()->query('SELECT type, slug, title, body FROM posts
    ...
    foreach ($st as $p) {
        ...
        if ($p['type'] === 'link') {
            $items[] = '<a href="' . e(trim($p['body'])) . '">' . e($p['title']) . '</a>';
            continue;
        }
