...
function menu_html(string $activeSlug = ''): string
{
    ...
    $rows = db()->query('SELECT id, type, slug, title, body, position, parent FROM posts
    ...
    $link = function (array $r) use ($activeSlug) {
        if ($r['type'] === 'link') {
            return '<a href="' . e(trim($r['body'])) . '">' . e($r['title']) . '</a>';
        }
...
function render(string $pageTitle, string $content, string $activeSlug = '', array $seo = []): never
{
    ...
    if (str_contains($vars_menu = menu_html($activeSlug), 'menu-drop')) {
        $head .= "\n" . '<style>.menu-sub{position:relative;display:inline-block}'
               ...
               . '.menu-drop a{display:block;padding:.35em 1em;margin:0!important;white-space:nowrap}</style>';
    }
...
function view_admin_list(string $type): never
{
    ...
    foreach ($st as $p) {
        ...
        $rows .= '<tr>
            ...
            <td class="actions">
              <a href="' . e($p['type'] === 'link' ? trim($p['body']) : url($p['slug'])) . '">View</a> &nbsp;
...
function parent_select(int $current, int $selfId): string
{
    ...
    $st = db()->query('SELECT id, title FROM posts WHERE type = "page" AND parent = 0
...
function view_admin_edit(?array $post, string $type): never
{
    ...
    if ($type === 'link') {
      ...
      <p class="lede">A menu entry that points anywhere — another site, a file, a download.</p>
      <form method="post" action="' . e(url('admin/save')) . '">
        ...
        <label>URL <input name="body" required placeholder="https://…"
        ...
        <label>Menu parent <span class="hint">(nest under a page)</span>
          ' . parent_select((int)($post['parent'] ?? 0), 0) . '</label>
...
function admin_save(): never
{
    ...
    if ($parent > 0) {
        $st = db()->prepare('SELECT id FROM posts WHERE id = ? AND type = "page"
                             AND parent = 0 AND id != ?');
    ...
    if ($id) {
        $st = db()->prepare('UPDATE posts SET title=?, slug=?, body=?, published=?, position=?,
                             show_title=?, description=?, allow_comments=?, parent=?, updated=? WHERE id=?');
        $st->execute([$title, $slug, $body, $published, $position, $showTitle, $description,
                      $allowComments, $parent, now(), $id]);
        if ($parent > 0) {
        ...
        $st = db()->prepare('INSERT INTO posts (type, slug, title, body, published, position,
                             show_title, description, allow_comments, parent, created, updated)
                             VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
        $st->execute([$type, $slug, $title, $body, $published, $position, $showTitle, $description,
                      $allowComments, $parent, now(), now()]);
