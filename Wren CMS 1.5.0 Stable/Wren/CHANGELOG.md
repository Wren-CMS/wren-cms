...
- New: optional two-factor authentication for the admin. Standard TOTP
  (RFC 6238), so any authenticator app works — Google Authenticator, Authy,
  1Password and the rest. Verified against the RFC's official test vectors
- Setup requires a working code before 2FA is switched on, so you cannot lock
  yourself out; eight single-use recovery codes are issued (shown once,
  stored hashed) for a lost phone
- Used codes cannot be replayed, one step of clock drift is tolerated,
  failed attempts are throttled, and turning 2FA off requires your password
